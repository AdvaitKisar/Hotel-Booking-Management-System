...
# Path
path = f'D:/Advait/Non Core/Python/Personal Projects/Hotel Booking Management System/Customer Data of Hotel {hotel_name}.xlsx'
...
if isFile == True:
    ...
    rest_counter = [sheet1.iloc[:, 16][i] for i in range(len(sheet1.iloc[:, 16]))]
else:
    name_list = []
    print(name_list)
    phno_list = []
...
start_date = todays_date# Booking starts from tomorrow till next 180 days
...
def booking(): # Function for booking a room
    ...
    rest_counter.append(0)
    # Taking details of customers
    n_p = int(input("Enter total no. of people: "))
    for i in range(n_p):
        while True:
            name = input("Enter the Full Name of Person No. {} : ".format(i+1))
            ...
            if name != '' and Mobile_no >= pow(10, 9) and Mobile_no < pow(10, 10) and address != '':
                ...
                print("The Fields of Name, Address and Mobile No. are mandatory and Mobile no. should be a 10 digit no.")
...
def restaurant(): # Function for restaurant
    ...
    for i in range(len(cust_list)):
        if cust_list[i] == CI:
            CI_ind = 1
            break
    Ph_no = int(input("Enter Mobile Number: "))
    while True:
        if Ph_no >= pow(10, 9) and Ph_no < pow(10, 10):
            break
        else:
            print("Enter the correct Mobile No.")

    if CI_ind == 1 and phno_list[CI_index] == Ph_no: # If customer ID is valid, the program will display the menu from text file
        rc = 0
        ...
        rest_counter[CI_index] += 1
        if rc != 0: # Prints the bill summary if sum is not zero and saves it as a text file as well
            with open(f'Restaurant Bill of Cust. ID {CI} - {name_list[CI_index]} [{rest_counter[CI_index]}].txt', 'a') as b:
...
def output(): # Printing output on the terminal
    ...
    print(f"Mobile No.s: {phno_list}")
    print(f"Room No.s: {room_no_list}")
    print(f"Customer IDs: {cust_list}")
    print(f"Check-in Dates: {checkin_list}")
    print(f"Check-out Dates: {checkout_list}")
    print(f"Room Charges: {room_charges}")
    print(f"Restaurant Charges: {rest_charges}")
    print(f"Other Service Charges: {other_charges}")
    print(f"Net Charges: {net_price_list}")
...
def record(): # Prints the record on terminal
    password = 'Southern_Crest_Chennai_1990'
    count = 0
    while count < 3:
        inp = input("Enter the Password: ")
        if inp != password:
            print("Invalid password. Try again")
            print(f"{2-count} attempts left.")
            if count == 2:
...
def other_services(): # Function for other services
    ...
    if other_svcs_inp == 1: # Function for cab service
        ...
        while True:
            if Ph_no >= pow(10, 9) and Ph_no < pow(10, 10):
                ...
                print("Enter the correct Mobile No.")
        if CI_ind == 1 and phno_list[CI_index] == Ph_no:
            os_price = 0
    ...
    elif other_svcs_inp == 3: # Function for Spa
        CI = int(input("Enter your Customer ID: "))
        if CI not in cust_list:
            print("You have entered an invalid Customer ID.")
            print("Please re-enter correct customer ID.")
